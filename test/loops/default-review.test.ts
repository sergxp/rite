...
describe("checkMemoryCompliance", () => {
  ...
  it("fails open when the reviewer returns nothing", async () => {
    ...
    const r = await checkMemoryCompliance("response", memories, config)
  ...
  it("parses a JSON pass verdict", async () => {
    ...
    expect((await checkMemoryCompliance("r", memories, config)).passed).toBe(true)
  })
  ...
  it("parses a JSON fail verdict with feedback", async () => {
    ...
    const r = await checkMemoryCompliance("r", memories, config)
  ...
  it("returns pass without calling the reviewer when aborted", async () => {
    ...
    ac.abort()
    const r = await checkMemoryCompliance("r", memories, config, ac.signal)
    expect(r.passed).toBe(true)
