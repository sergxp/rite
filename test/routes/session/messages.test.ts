...
import {
  ...
  TRANSCRIPT_LOAD_MORE_STEP,
  prepareMarkdown,
  renderMarkdownLines,
  transcriptWindow,
} from "../../../src/routes/session/messages"
...
describe("transcript windowing", () => {
  it("shows all items when transcript is within the window size", () => {
    ...
    const { visible, hiddenBefore } = transcriptWindow(items, start)
  ...
  it("windows long transcripts to the tail on initial load", () => {
    ...
    const start = naturalStart(items.length)
    const { visible, hiddenBefore, hiddenAfter } = transcriptWindow(items, start)

    expect(hiddenBefore).toBe(200 - TRANSCRIPT_WINDOW_SIZE)
    expect(hiddenAfter).toBe(0)
    expect(visible).toHaveLength(TRANSCRIPT_WINDOW_SIZE)
    expect(visible.at(-1)).toEqual(items[199])
    expect(visible[0]).toEqual(items[hiddenBefore])
  })

  it("windows long transcripts to the tail on the first resume render", () => {
    const items: DisplayItem[] = Array.from({ length: 979 }, (_, i) => ({
      kind: i % 2 === 0 ? "user" : "assistant",
      content: `turn ${i}`,
    } as DisplayItem))

    const { visible, hiddenBefore, hiddenAfter } = transcriptWindow(items, -1)

    expect(hiddenBefore).toBe(979 - TRANSCRIPT_WINDOW_SIZE)
    expect(hiddenAfter).toBe(0)
    expect(visible).toHaveLength(TRANSCRIPT_WINDOW_SIZE)
    expect(visible[0]).toEqual(items[hiddenBefore])
  })

  it("Ctrl+U moves the bounded window backward by TRANSCRIPT_LOAD_MORE_STEP", () => {
    const items: DisplayItem[] = Array.from({ length: 200 }, (_, i) => ({
      kind: "user",
      content: `turn ${i}`,
    } as DisplayItem))
    ...
    const { visible: v1, hiddenBefore: h1, hiddenAfter: later1 } = transcriptWindow(items, afterOne)
    expect(h1).toBe(afterOne)
    expect(later1).toBe(TRANSCRIPT_LOAD_MORE_STEP)
    expect(v1).toHaveLength(TRANSCRIPT_WINDOW_SIZE)

    // Simulate a second Ctrl+U press
    const afterTwo = Math.max(0, afterOne - TRANSCRIPT_LOAD_MORE_STEP)
    const { visible: v2, hiddenBefore: h2, hiddenAfter: later2 } = transcriptWindow(items, afterTwo)
    expect(h2).toBe(afterTwo)
    expect(later2).toBe(TRANSCRIPT_LOAD_MORE_STEP * 2)
    expect(v2).toHaveLength(TRANSCRIPT_WINDOW_SIZE)
  })
  ...
  it("Ctrl+U cannot go below index 0", () => {
    ...
    const { visible, hiddenBefore } = transcriptWindow(items, afterU)
    expect(hiddenBefore).toBe(0)
